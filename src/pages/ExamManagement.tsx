...
const ExamManagement: React.FC = () => {
  ...
  const loadExams = () => {
    const allExams = ExamStorage.getAllExams();
    setExams(allExams);
  };

  const handleDeleteExam = (examId: string) => {
    if (window.confirm('Are you sure you want to delete this exam?')) {
      ExamStorage.deleteExam(examId);
      loadExams();
    }
  };

  const toggleExamStatus = (examId: string) => {
    const exam = ExamStorage.getExamById(examId);
    if (exam) {
      const updatedExam = { ...exam, isActive: !exam.isActive };
      ExamStorage.updateExam(examId, updatedExam);
      loadExams();
    }
  };
                    ...
                    <tr key={exam.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <button
                          ...
                          className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full cursor-pointer hover:opacity-80 ${getStatusColor(exam.isActive)}`}
                        >
